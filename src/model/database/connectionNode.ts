...
export class ConnectionNode extends Node implements CopyAble {
  ...
  public iconPath: string | vscode.ThemeIcon = path.join(
    Constants.RES_PATH,
    "icon/server/mysql.svg"
  );
  ...
  public async getChildren(isRresh: boolean = false): Promise<Node[]> {
    ...
    const sql = hasCatalog
      ? this.dialect.showDatabases()
      : this.dialect.showSchemas();
    return this.execute<any[]>(sql).then((databases) => {
      const includeDatabaseArray = this.includeDatabases
        ?.toLowerCase()
        ?.split(",");
      const usingInclude =
        this.includeDatabases &&
        includeDatabaseArray &&
        includeDatabaseArray.length >= 1;
      const databaseNodes = databases
        .filter((db) => {
          if (usingInclude && !db.schema) {
            return (
              includeDatabaseArray.indexOf(db.Database.toLowerCase()) != -1
            );
          }
          if (this.hideSystemSchema) {
            if (
              ...
              (this.dbType == DatabaseType.PG &&
                db.schema &&
                ["pg_toast", "information_schema", "pg_catalog"].includes(
                  db.schema.toLowerCase()
                ))
            ) {
              return false;
            }
  ...
  private getLabel(parent: Node) {
    if (parent.name) {
      ...
      const preferName = Global.getConfig(
        ConfigKey.PREFER_CONNECTION_NAME,
        true
      );
      if (preferName) {
    ...
    this.label = this.usingSSH
      ? `${this.ssh.host}@${this.ssh.port}`
      : `${this.host}@${this.instanceName ? this.instanceName : this.port}`;
  ...
  private getIcon() {
    ...
    switch (this.dbType) {
      case DatabaseType.MYSQL:
        this.iconPath =
          basePath + (isActive ? "mysql_active.svg" : "mysql.svg");
        break;
      ...
      case DatabaseType.SQLITE:
        this.iconPath =
          basePath + (isActive ? "sqlite_active.svg" : "sqlite.svg");
        break;
      case DatabaseType.MONGO_DB:
        this.iconPath =
          basePath + (isActive ? "mongo_active.svg" : "mongo.svg");
        break;
    }
  }
  ...
  public createDatabase() {
    FileManager.showSQLTextDocument(
      this,
      this.dialect.createDatabase(""),
  ...
  public async deleteConnection(context: vscode.ExtensionContext) {
    Util.confirm(
      `Are you want to delete Connection ${this.label} ? `,
      async () => {
