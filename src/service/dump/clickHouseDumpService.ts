  ...
  public async dump(node: Node, withData: boolean) {
    ...
    if (commandExistsSync("pg_dump")) {
      ...
      if (folderPath) {
        ...
        const port = node.usingSSH
          ? NodeUtil.getTunnelPort(node.key)
          : node.port;
        ...
        if (node.password) {
          command =
            platform() == "win32"
              ? `set "PGPASSWORD=${node.password}" && ${command}`
              : `export PGPASSWORD='${node.password}' && ${command}`;
        }
          ...
          .then(() => {
            vscode.window
              .showInformationMessage(
                `Backup ${node.getHost()}_${node.schema} success!`,
                "open"
              )
              .then((action) => {
                if (action == "open") {
                  vscode.commands.executeCommand(
                    "vscode.open",
                    vscode.Uri.file(folderPath.fsPath)
                  );
                }
              });
          })
