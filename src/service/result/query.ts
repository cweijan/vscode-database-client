...
export class QueryPage {
      ...
      eventHandler: async (handler) => {
        handler
          .on("init", () => {
            ...
            const uglyPath = handler.panel.webview
              .asWebviewUri(
                Uri.file(Global.getExtPath("out", "webview", "ugly.jpg"))
              )
              .toString();
            handler.emit(queryParam.type, {
              ...queryParam.res,
              dbType: dbOption.dbType,
              single: queryParam.singlePage,
              language: env.language,
              fontFamily,
              fontSize,
              showUgly: Global.getConfig("showUgly", false),
              uglyPath
            });
          })
          .on("execute", (params) => {
          ...
          .on("next", async (params) => {
            ...
            const sql = ServiceManager.getPageService(dbOption.dbType).build(
              params.sql,
              params.pageNum,
              params.pageSize
            );
            dbOption.execute(sql).then((rows) => {
              ...
              handler.emit(MessageType.NEXT_PAGE, {
                sql,
                data: rows,
                costTime
              });
          ...
          .on("count", async (params) => {
            ...
            if (dbOption.dbType == DatabaseType.MONGO_DB) {
              ...
              let count_query = params.sql.replace(
                /(?<=SELECT)(\s.*?)+(?=FROM)/gim,
                " count(*) count "
              );

              dbOption.execute(count_query).then((rows) => {
          ...
          .on("export", (params) => {
            this.exportService
              .export({
                ...params.option,
                request: queryParam.res.request,
                dbOption
              })
              .then(() => {
          ...
          .on("openGithub", () => {
            env.openExternal(
              Uri.parse("https://github.com/cweijan/vscode-database-client")
            );
          })
          .on("openCoffee", () => {
          ...
          .on("dataModify", () => {
            if (handler.panel.title.indexOf("*") == -1)
              handler.panel.title = `${handler.panel.title}*`;
          })
          .on("saveModify", (sql) => {
            dbOption
              .execute(sql)
              ...
              .catch((err) => {
                QueryPage.send({
                  ...
                  queryOption: queryParam.queryOption,
  ...
  private static async adaptData(queryParam: QueryParam<any>) {
    switch (queryParam.type) {
      case MessageType.DATA:
        ...
        const pageSize = ServiceManager.getPageService(
          queryParam.connection.dbType
        ).getPageSize(queryParam.res.sql);
        (queryParam.res as DataResponse).pageSize =
          queryParam.res.data?.length && queryParam.res.data.length > pageSize
            ? queryParam.res.data.length
            : pageSize;
        break;
  ...
  private static isActiveSql(option: QueryOption): boolean {
    ...
    if (!activeDocument || option.split === false) {
      return false;
    }
    ...
    return (
      ...
      extName == ".sql" ||
      extName == ".es" ||
      fileName.match(/mock.json$/) != null
    );
  }

  private static async handleHtml(
    html: string,
    ...
    switch (resourceRoot) {
      ...
      case "backup":
        return html
          .replace(
            "../webview/js/query.js",
            `https://cdn.jsdelivr.net/npm/vscode-mysql-client2@${Constants.CDN_VERSION}/out/webview/js/query.js`
          )
          .replace(
            "../webview/js/vendor.js",
            `https://cdn.jsdelivr.net/npm/vscode-mysql-client2@${Constants.CDN_VERSION}/out/webview/js/vendor.js`
          );
      case "internalServer":
      default:
        ...
        return html
          .replace(
            "../webview/js/query.js",
            `http://127.0.0.1:${ResourceServer.port}/query.js`
          )
          .replace(
            "../webview/js/vendor.js",
            `http://127.0.0.1:${ResourceServer.port}/vendor.js`
          );
    }
    return html;
  }
  ...
  private static async loadMongoColumnList(
    queryParam: QueryParam<DataResponse>
  ) {
    const parse = queryParam.res.sql.match(
      /db\('(.+?)'\)\.collection\('(.+?)'\)/
    );
  ...
  private static async loadColumnList(queryParam: QueryParam<DataResponse>) {
    ...
    if (
      queryParam.connection.dbType == DatabaseType.MSSQL &&
      tableName.indexOf(".") != -1
    ) {
    ...
    if (tableNode) {
      ...
      let primaryKeyList = [];
      const columnList = (await tableNode.getChildren(true)).map(
